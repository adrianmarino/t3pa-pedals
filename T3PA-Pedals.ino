...
void updateConfig() {
  if (Serial.available() > 0) {
    ...
    if (value.equals("help")) {
      ...
      Serial.println("Load json settings");
      ...
      if (loadJsonError) {
        ...
        Serial.println("Set save settings");
...
void updateJoystick() {
  Joystick.setZAxis(int(clutchFilter->filter(readAnalogInput(CLUTCH)))); 
  Joystick.setRxAxis(int(breakFilter->filter(readAnalogInput(BREAK))));
  Joystick.setThrottle(int(throttleFilter->filter(readAnalogInput(THROTTLE))));
}
